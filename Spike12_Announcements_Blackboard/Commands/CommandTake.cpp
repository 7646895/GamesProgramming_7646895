...
void CommandTake::run(vector<string> input, Player *p)
{
    if(input.at(1) == "all")
    {
        MoveObjectMessage *moMsg = new MoveObjectMessage(1);
        vector<string> objs;
        vector<GameObject*> currentLocInv = p->getCurrentLocation()->getInventoryObject()->getInventory();
        for(auto i=currentLocInv.begin(); i!=currentLocInv.end(); i++ )
        {
            objs.push_back((*i)->getName());
        }
        Message *msg = (Message*)moMsg;
        int effect = _msgHandler->sendMoveAnnouncement(msg, msg->getType(), objs);
        vector<string> vString;
        vString.push_back("player");
        sendHealthMessage("decrease", effect, vString);
    }
    if(input.capacity() == 2)
    {
