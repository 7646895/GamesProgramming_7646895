...
CommandAttack::CommandAttack(MessageHandler *msgHandler)
{
...
void CommandAttack::run(vector<string> input, Player *p)
{
    ...
    if(input.capacity() == 2)
    {
        if(attackVectorContains(attackObj->getVector(), "me"))
        {
            ...
            tempV.push_back("player");
            sendHealthMessage("decrease", pDamage->getDamage(), tempV);
            //gameObj health decreases by p damage
            tempV.clear();
            tempV.push_back(gameObj->getName());
            sendHealthMessage("decrease", pDamage->getDamage(), tempV);
            //oHealth->decreaseHealth();
            cout << "You've attacked the " << monster << "!\n";
            _msgHandler->updateBlackboard();
        }
        else
        {
